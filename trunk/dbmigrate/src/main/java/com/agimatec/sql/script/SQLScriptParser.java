...
public class SQLScriptParser {
     ...
     * @throws IOException  - error accessing the script file (e.g. FileNotFound)
     */
    public void iterateSQLLines(ScriptVisitor visitor, URL url)
            throws SQLException, IOException {
        if (getLog().isInfoEnabled()) {
            getLog().info("Executing line by line " + url + " ... ");
        }
        BufferedReader input = getURLReader(url);
        try {
            iterateSQLLines(visitor, input);
        } finally {
            input.close();
    ...
    public void execSQLScript(ScriptVisitor visitor, URL url)
            throws IOException, SQLException {
        Reader input = getURLReader(url);
        if (getLog().isInfoEnabled()) {
            getLog().info("Reading and executing " + url + " ... ");
        }
        String statement = IOUtils.toString(input);
        statement = finish(statement);
        statement = fixLF(statement);
