...
public class DDLParserTool {
    ...
    private void start() throws Exception {
        ...
        DDLScriptSqlMetaFactory factory =
                new DDLScriptSqlMetaFactory(expressions);
        for (String script : scriptsToParse) {
            factory.fillCatalog(new URL(script));
        }
    ...
    private void checkValid() {
        if (catalog == null) {
            throw new IllegalArgumentException(
                    "no catalog output file name given with option -catalog");
        }
        if (dbms == null) {
        ...
        if (scriptsToParse.isEmpty()) {
            throw new IllegalArgumentException(
                    "no scripts to parse given with option -parse");
        }
    }

    private static void printUsage() {
        System.out.println("usage: java " + DDLParserTool.class.getName() +
        ...
        System.out.println(
                "\t-parse\t (required) can appear multiple times. a URL of a script to parse into the catalog, e.g. file:create-tables.sql - All scripts must belong to the same database schema catalog");
    }
}
