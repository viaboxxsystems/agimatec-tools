...
public class ScriptTransformator extends DDLScriptSqlMetaFactory {
    ...
    public ScriptTransformator(DDLExpressions ddlSpecification, PrintWriter target,
    ...
    public int visitStatement(String statement) throws SQLException {
        ...
        for (ExtractExpr theExpr : getDdlSpec().getExpressions()) {
            ...
            if (values != null) {
                ...
                break; // we stop when the first expression matches. 
            }
        }
        if (found == 0) {
            ...
            for (Object column : columns) {
                ...
                if (map.containsKey("add-column")) { // add column
                    ...
                    genAlterColumnType(map);
                        ...
                        "alter-column-drop-notnull")) { // alter-column-drop-notnull
                    genColumnDropNotNull(map);
                } else if (map.containsKey(
                        "alter-column-set-notnull")) { // alter-column-set-notnull
                    genColumnSetNotNull(map);
                } else if (map.containsKey("constraint")) { // add-constraint
                    genAddConstraint(map, values);
                } else if (map.containsKey("add-foreign-key")) { // add foreign key
                    genAddForeignKey(map, values);
                } else if (map.containsKey("drop-column")) { // drop-column
                    genDropColumn(map);
                } else if (map.containsKey("drop-constraint")) { // drop-constraint
